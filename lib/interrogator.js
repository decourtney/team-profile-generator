...
class Interrogator
{
        ...
        if (val === 'employee')
        {
            return [
                ...
                },
                {
                    type: 'input',
                    name: 'name',
                    message: `Enter the employee First and Last Name:`,
                    validate: this.#validateIsName
                },
                {
                    type: 'input',
    ...
    #validateIsID(val)
    {
        console.log(this.usedIDNumbers)
        ...
        // After passing valid number check, If there are no used numbers add val to the usedIDNumbers array and return
        if (this.usedIDNumbers == undefined)
        {
            this.usedIDNumbers = val;
            return true;
        }

        // After verifying a 3 digit number AND the usedIDNumbers array is populated, check if current number is already in use
        if (this.usedIDNumbers.includes(val))
        {
            return 'That ID# is already in use.'
        }

        this.usedIDNumbers.push(val);
        return true;
    }
