...
class Interrogator
{
    ...
    async loadMenu(title)
    {
        ...
        title('main');
        console.log(chalk.yellow(`\t\t\tWelcome, ${manager.name.split(' ')[0]}! Lets build your winning team!\n`));
        ...
        while (true)
        {
            ...
            title('main');
            console.log(chalk.yellow(`\t\t\t${answers.name} has been added to the team.\n`))
        }
    ...
    async #getTeamMember(role)
    {
        ...
        {
            case 'Manager':
                ques.push({ type: 'input', name: 'extraQuestion', message: chalk.yellowBright('Enter the employee office number:'), validate: this.#validateIsPhone });
                break;
            case 'Engineer':
                ques.push({ type: 'input', name: 'extraQuestion', message: chalk.yellowBright('Enter the employee GitHub username'), validate: this.#validateIsGithubName });
                break;
            case 'Intern':
                ques.push({ type: 'input', name: 'extraQuestion', message: chalk.yellowBright('Enter the employee school name'), validate: this.#validateIsValue });
                break;
            default:
                break;
        }
            ...
            return [
                {
                    type: 'input',
                    name: 'id',
                    message: chalk.yellowBright('Enter a 3 digit employee ID# (No leading Zeros(0)):'),
                    validate: this.#validateIsID
                },
                {
                    type: 'input',
                    name: 'name',
                    message: chalk.yellowBright(`Enter the employee name:`),
                    validate: this.#validateIsName
                },
                {
                    type: 'input',
                    name: 'email',
                    message: chalk.yellowBright('Enter the employee email:'),
        ...
        } else
        {
            return [
                ...
                },
                {
                    type: 'list',
                    name: 'employeeRole',
                    message: chalk.yellowBright('Choose the team members role'),
                    choices: ['Engineer', 'Intern'],
                },
                {
                    type: 'confirm',
                    name: 'isExit',
                    message: chalk.yellowBright('You haven\'t added any team members.\n\tAre you sure you want to exit?'),
                    default: false
                },
            ];
        }
    ...
    #validateIsID(val)
    {
        ...
        {
            return chalk.redBright('Please enter a 3 digit employee ID# (No leading Zeros(0)');
        }
    ...
    #validateIsGithubName(val)
    {
        ...
        {
            return chalk.redBright('Please enter a valid GitHub username.');
        }
    ...
    #validateIsValue(val)
    {
        ...
        if (!val)
        {
            return chalk.redBright('Please enter a value');
        }
